...
class FeatureExtractor:
    """
    Extracts foreground and background features from images using CAMs and MedCLIP.
    
    This class implements the feature extraction pipeline:
    1. Resizes CAMs and images to CLIP input size (224x224)
    2. Applies adaptive thresholding to create binary masks
    3. Separates images into foreground/background regions
    4. Extracts features using MedCLIP vision model
    """
    def __init__(self, mask_adapter, clip_size=224):
        ...
        Args:
            ...
            clip_size: Target size for CLIP model input (default: 224)
        """
    ...
    def prepare_cam_mask(self, cam, N):
        ...
        Example with batch_size=8, 12 subclasses:
            ...
            Process:
                1. Resize to CLIP size: cam_224 [8, 12, 224, 224] (if already 224, no change)
        ...
        Returns:
            cam_224: Resized and reshaped CAMs, shape [N*num_classes, 1, clip_size, clip_size]
            cam_224_mask: Binary masks from adaptive thresholding, same shape as cam_224
        """
        # Resize CAMs to CLIP input size (224x224)
    ...
    def prepare_image(self, img):
        """
        Resizes input images to CLIP input size.
        
        Example:
            Input: img [8, 3, 512, 512] - batch of 8 RGB images at original resolution
            Output: [8, 3, 224, 224] - resized to CLIP input size
        
        Args:
            img: Input images, shape [batch_size, C, H, W]
        
        Returns:
            Resized images, shape [batch_size, C, clip_size, clip_size]
        """
    ...
    def get_masked_features(self, fg_features, bg_features, fg_masks, bg_masks, clip_model):
        """
        Extracts feature vectors from foreground and background regions using MedCLIP.
        
        This applies the binary masks to zero out irrelevant regions, then passes
        the masked images through MedCLIP's vision model to get feature vectors.
        ...
        Example with 3 positive samples:
            ...
            Process:
                ...
                2. Pass through MedCLIP vision model:
                   - Vision model processes [3, 3, 224, 224] -> [3, 512] (global feature vector)
        ...
        Args:
            ...
            fg_masks: Foreground binary masks, shape [N, 1, H, W]
            bg_masks: Background binary masks, shape [N, 1, H, W]
            clip_model: MedCLIP model with vision_model attribute
        ...
        # MedCLIP vision model processes the masked images and outputs global feature vectors
        # Example: [3, 3, 224, 224] -> [3, 512] (one 512-dim vector per sample)
        fg_img_features = clip_model.vision_model(fg_features * fg_masks)  # [N, D]
        bg_img_features = clip_model.vision_model(bg_features * bg_masks)  # [N, D]
    ...
    def process_batch(self, inputs, cam, label, clip_model):
        ...
        Example with batch_size=8, 12 subclasses, 3 channels:
            Input:
                ...
                clip_model: MedCLIP model instance
        ...
        # Applies masks to zero out irrelevant regions, then passes through vision model
